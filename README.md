...
**Текущее состояние CI**: `python -m benchmarks.score` **красный** из-за
`PASSPORT_ISSUER` recall 0.0 (см. `docs/qa-report.md`, дефект #6). CI не
заявляется зелёным, пока этот gate не пройден. `docker compose config`
...
- **Известный дефект детекции**: `PASSPORT_ISSUER` имеет recall 0.0 в
  локальном бенчмарке (regex обрезает название органа до 5 слов, поэтому span
  не совпадает с эталоном). Бенчмарк это фиксирует и падает (per-type recall
  gate), пока владелец детекции не исправит правило.
...
также false positives на негативных кейсах. Бенчмарк падает, если общий F1
ниже порога **или** если у любого типа с эталонными кейсами recall ниже
`_MIN_RECALL_PER_TYPE` (0.1) — так общий зелёный результат не маскирует
нулевой recall обязательного типа. Не заявляйте качество 95% без реального
прогона этого бенчмарка.
